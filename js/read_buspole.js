...
const buspole_datashaping = async (list_buspole_data, use_list) => {
  ...
  list_buspole_data.forEach(async (buspole) => {
    ...
    // 創価大学周辺のバス停か否かを判定
    if (use_list.start_list.includes(buspole["dc:title"])) {
      // データの結合
  ...
  const dict_buspole_data = { route_list: list_busroute, route_start: buspole_start, route_goal: buspole_goal_push };
